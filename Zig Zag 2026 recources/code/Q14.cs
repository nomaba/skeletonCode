...
namespace AntSimCS          //Q14
{
    class Program
    {
        ...
        static void Main()
        {
            ...
            do
            {
                ...
                switch (Choice)
                {
                    ...
                    //CHANGE
                    case "6":
                        int PredatorRow = 0, PredatorColumn = 0;
                        GetCellReference(ref PredatorRow, ref PredatorColumn);
                        ThisSimulation.AddPredatorToCell(PredatorRow, PredatorColumn);
                        Console.WriteLine($"Predator added to cell {PredatorRow}, {PredatorColumn}{Environment.NewLine}");
                        break;
                        //END CHANGE
                }
            } while (Choice != "9");
        ...
        static void DisplayMenu()
        {
            ...
            //CHANGE
            Console.WriteLine("6. Put Predator into cell");
            //END CHANGE
        ...
        class Simulation
        {
            ...
            //CHANGE
            protected List<Predator> Predators = new List<Predator>();
            //CHANGE
            ...
            //CHANGE
            public void AddPredatorToCell(int Row, int Column)
            {
                Predators.Add(new Predator(Row, Column));
            }
            //END CHANGE
            ...
            public void UpdateAntsPheromoneInCell(Ant A)
            {
                ...
                foreach (Pheromone P in Pheromones)
                {
                    ...
                    {
                        if (A.GetTypeOfAnt() == "soldier" && A.GetDetails() == "engaging")
                        {
                            P.UpdateStrength(NewPheromoneStrength * 5); //soldier ants leave stronger pheromones when engaging predators
                        }
                        else
                        {
                ...
                if (A.GetTypeOfAnt() == "soldier" && A.GetDetails() == "engaging")
                {
                    Pheromones.Add(new Pheromone(A.GetRow(), A.GetColumn(), A.GetID(), NewPheromoneStrength * 5, PheromoneDecay)); //soldier ants leave stronger pheromones when engaging predators
                }
                else
                {
            ...
            public int GetNumberOfAntsInCell(Cell C)
            {
                ...
                {
                    if (A.InSameLocation(C) && (A.GetTypeOfAnt() == "worker" || A.GetTypeOfAnt() == "queen"))
                    {
                        Count++;
                    }
                }
            ...
            public int GetNumberOfSoldierAntsInCell(Cell C)
            {
                int Count = 0;
                foreach (Ant A in Ants)
                {
                    if (A.InSameLocation(C) && A.GetTypeOfAnt() == "soldier")
                    {
                        Count++;
                    }
                }
            ...
            public int GetNumberOfPredatorsInCell(Cell C)
            {
                int Count = 0;
            ...
            public string GetDetails()
            {
                ...
                for (int Row = 1; Row <= NumberOfRows; Row++)
                {
                    for (int Column = 1; Column <= NumberOfColumns; Column++)
                    {
                        ...
                        //CHANGE
                        int NumberOfSoldierAnts = GetNumberOfSoldierAntsInCell(TempCell);
                        if (NumberOfSoldierAnts > 0)
                        {
                            Details += $"| Soldier Ants: {NumberOfSoldierAnts} |  ";
                        }
                        int NumberOfPredators = GetNumberOfPredatorsInCell(TempCell);
                        if (NumberOfPredators > 0)
                        {
                            Details += $"| Predators: {NumberOfPredators} |  ";
                        }
                        //END CHANGE
            ...
            public void AdvanceStage(int NumberOfStages)
            {
                for (int Count = 1; Count <= NumberOfStages; Count++)
                {
                    ...
                    foreach (Ant A in Ants)
                    {
                        ...
                        //CHANGE
                        else if (CurrentCell.GetAmountOfFood() > 0 && A.GetFoodCarried() == 0 && A.GetFoodCapacity() > 0 && A.GetTypeOfAnt() != "soldier")
                        {
                            //END CHANGE
                            int FoodObtained;
                            do
                        ...
                        else
                        {
                            ...
                            //CHANGE
                            if (A.GetTypeOfAnt() == "soldier" && A.GetDetails() == "patrolling")
                            {
                                List<int> NeighbourIndices = GetIndicesOfNeighbours(A.GetRow(), A.GetColumn());
                                foreach (int Index in NeighbourIndices)
                                {
                                    if (Index != -1)
                                    {
                                        Cell TempCell = Grid[Index];
                                        foreach (Predator PotentialPredator in Predators)
                                        {
                                            if (PotentialPredator.InSameLocation(TempCell))
                                            {
                                                A.ChooseCellToMoveTo(NeighbourIndices, Index);
                                                A.UpdateState("engaging"); //engage with predator
                                                Console.WriteLine("Soldier Ant engaging with predator");
                                            }
                                        }
                                    }
                                }
                            }
                            else if (A.GetTypeOfAnt() == "soldier" && A.GetDetails() == "engaging" && CurrentCell.InSameLocation(A))
                            {
                                UpdateAntsPheromoneInCell(A);
                                Console.WriteLine("Soldier Ant is fighting with predator");
                            }
                            else if (A.GetTypeOfAnt() == "soldier" && A.IsAtOwnNest())
                            {
                                ...
                                A.UpdateState("patrolling");//Set the soldier's action to patrolling when it leaves the nest
                            }
                            //END CHANGE
                            else
                            {
        ...
        {
            public Predator(int StartRow, int StartColumn) : base(StartRow, StartColumn)
            {
            }
            public override string GetDetails()
            {
                return $"{base.GetDetails()} Predator present";
            }
        }
        //END CHANGE
        ...
        class SoldierAnt : Ant
        {
            ...
            public SoldierAnt(int StartRow, int StartColumn, int NestInRow, int NestInColumn)
            : base(StartRow, StartColumn, NestInRow, NestInColumn)
            {
                TypeOfAnt = "soldier";
                FoodCapacity = 5;
