...
namespace AntSimCS      //Q19
{
    class Program
    {
        ...
        static void Main()
        {
            ...
            do
            {
                ...
                switch (Choice)
                {
                    ...
                    //CHANGE
                    case "6":
                        int DropRow = 0, DropColumn = 0;
                        GetCellReference(ref DropRow, ref DropColumn);
                        ThisSimulation.RemovePheromonesFromCell(DropRow, DropColumn);
                        Console.WriteLine($"Cinnamon dropped on cell {DropRow}, {DropColumn} to remove pheromones.{Environment.NewLine}");
                        break;
                    //END CHANGE
                }
            } while (Choice != "9");
        ...
        static void DisplayMenu()
        {
            ...
            //CHANGE
            Console.WriteLine("6: Drop Cinnamon onto a cell to remove pheromones");
            //END CHANGE
        ...
        class Simulation
        {
            ...
            private List<int> GetIndicesOfNeighbours(int Row, int Column)
            {
                ...
                foreach (int RowDirection in new int[] { -1, 0, 1 })
                {
                    foreach (int ColumnDirection in new int[] { -1, 0, 1 })
                    {
                        ...
                        {
                            //CHANGE
                            if (Grid[GetIndex(NeighbourRow, NeighbourColumn)].ContainsCinnamon())
                            {
            ...
            {
                //CHANGE
                if (Grid[GetIndex(A.GetRow(), A.GetColumn())].ContainsCinnamon())
                {
                    return;
                }
                //END CHANGE
            ...
            //CHANGE
            public void RemovePheromonesFromCell(int Row, int Column)
            {
                Cell TargetCell = Grid[GetIndex(Row, Column)];
                TargetCell.PlaceCinnamon();
                List<Pheromone> PheromonesToRemove = new List<Pheromone>();
                foreach (Pheromone P in Pheromones)
                {
                    if (P.InSameLocation(TargetCell))
                    {
                        PheromonesToRemove.Add(P);
                    }
                }
                foreach (Pheromone P in PheromonesToRemove)
                {
                    Pheromones.Remove(P);
                }
            }
        ...
        class Cell : Entity
        {
            ...
            //CHANGE
            private bool HasCinnamon = false;
            //END CHANGE
            ...
            //CHANGE
            public void PlaceCinnamon()
            {
                AmountOfFood = 0;
                HasCinnamon = true;
            }
            public bool ContainsCinnamon()
            {
                return HasCinnamon;
            }
            //END CHANGE
