...
namespace AntSimCS              //Q8
{
    class Program
    {
        ...
        static void Main()
        {
            ...
            do
            {
                ...
                switch (Choice)
                {
                    ...
                    //CHANGE
                    case "6":
                        int AntOriginalRow = 0, AntOriginalColumn = 0, AntRelocationRow = 0, AntRelocationColumn = 0;
                        Console.WriteLine("Give the location of the ant that you want to relocate: ");
                        GetCellReference(ref AntOriginalRow, ref AntOriginalColumn);
                        Console.WriteLine("Give the location that you would like to relocate the ant to: ");
                        GetCellReference(ref AntRelocationRow, ref AntRelocationColumn);
                        Console.WriteLine(ThisSimulation.RelocateAnt(AntOriginalRow, AntOriginalColumn, AntRelocationRow, AntRelocationColumn));
                        break;
                        //END CHANGE
                }
            } while (Choice != "9");
        ...
        static void DisplayMenu()
        {
            ...
            Console.WriteLine("6: Relocate an ant to a new cell on the grid");
        ...
        class Simulation
        {
            ...
            //CHANGE
            public string RelocateAnt(int AntOriginalRow, int AntOriginalColumn, int AntRelocationRow, int AntRelocationColumn)
            {
                foreach (Ant A in Ants)
                {
                    if (A.GetRow() == AntOriginalRow && A.GetColumn() == AntOriginalColumn && A.GetTypeOfAnt() == "worker")
                    {
                        A.SetNewLocation(AntRelocationRow, AntRelocationColumn);
                        return $"Ant relocated successfully from {AntOriginalRow},{AntOriginalColumn} to {AntRelocationRow},{AntRelocationColumn}.";
                    }
                }
                return "No ant found at the original location.";
            }
            //END CHANGE
        ...
        class Ant : Entity
        {
            ...
            //CHANGE
            public virtual void SetNewLocation(int NewRow, int NewColumn)
            {
        ...
        class WorkerAnt : Ant
        {
            ...
            //CHANGE
            public override void SetNewLocation(int NewRow, int NewColumn)
            {
                Row = NewRow;
                Column = NewColumn;
            }
