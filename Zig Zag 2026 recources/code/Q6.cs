    ...
    class Program
    {
        ...
        class Simulation
        {
            ...
            public void SetUpANestAt(int Row, int Column)
            {
                ...
                {
                    if (Ants[i].GetTypeOfAnt() == "flying" && Ants[i].GetRow() == Row && Ants[i].GetColumn() == Column)
                    {
                        Ants.RemoveAt(i);
                        break;
                    }
                }
                Nests.Add(new Nest(Row, Column, StartingFoodInNest, NumberOfRows, NumberOfColumns));
                //END CHANGE
            ...
            public void AdvanceStage(int NumberOfStages)
            {
                for (int Count = 1; Count <= NumberOfStages; Count++)
                {
                    ...
                    for (int i = 0; i < Ants.Count; i++)
                    {
                        ...
                        //CHANGE
                        else if (CurrentCell.GetAmountOfFood() > 0 && A.GetFoodCarried() == 0 && A.GetFoodCapacity() > 0 && A.GetTypeOfAnt() != "flying")
                        {
                            int FoodObtained;
                            do
                            {
                        ...
                        if (A.GetTypeOfAnt() == "flying" && !A.IsAtOwnNest())
                        {
                            Console.WriteLine($"Flying ant has flown to cell {A.GetRow()}, {A.GetColumn()} to create a new nest");
                            SetUpANestAt(A.GetRow(), A.GetColumn());
                        }
                        //END CHANGE
                    }
                    foreach (Nest N in Nests)
        ...
        class FlyingAnt : Ant
        {
            ...
            public FlyingAnt(int StartRow, int StartColumn, int NestInRow, int NestInColumn, int SimWidth, int SimHeight)
                : base(StartRow, StartColumn, NestInRow, NestInColumn)
            {
                TypeOfAnt = "flying";
                FoodCapacity = 5;
                SimulationWidth = SimWidth;
                SimulationHeight = SimHeight;
            }
            ...
            public override void AdvanceStage(List<Nest> Nests, List<Ant> Ants, List<Pheromone> Pheromones)
            {
                ...
                if (Stages == 3)
                {
                    int RNoRow, RNoColumn;
                    bool ValidNewLocation;
                    do
                    {
                        ValidNewLocation = true;
                        RNoRow = RGen.Next(1, SimulationHeight);
                        RNoColumn = RGen.Next(1, SimulationWidth);
                        foreach (Nest N in Nests)
                        {
                            if (N.GetRow() == RNoRow && N.GetColumn() == RNoColumn)
                            {
                                ValidNewLocation = false;
                            }
                        }
                    } while (!ValidNewLocation);
                    Row = RNoRow;
                    Column = RNoColumn;
                    HasFlown = true;
                    Console.WriteLine("Flying Ant has flown the nest!");
                }
            }
            public override bool IsAtOwnNest()
            {
                if (HasFlown)
                {
                    return false;
        ...
        class Nest : Entity
        {
            ...
            public Nest(int StartRow, int StartColumn, int StartFood, int SimWidth, int SimHeight) : base(StartRow, StartColumn)
            {
                FoodLevel = StartFood;
                NumberOfQueens = 1;
                ID = NextNestID;
            ...
            public override void AdvanceStage(List<Nest> Nests, List<Ant> Ants, List<Pheromone> Pheromones)
            {
                ...
                else
                {
                    for (int A = 1; A <= NumberOfQueens; A++)
                    {
                        ...
                        //CHANGE
                        int RNo3 = RGen.Next(0, 100);
                        if (RNo3 < 20)
                        {
                            Console.WriteLine("Flying Ant born into the nest!");
                            Ants.Add(new FlyingAnt(Row, Column, Row, Column, SimulationWidth, SimulationHeight));
                        }
                        //END CHANGE
                    }
                }
