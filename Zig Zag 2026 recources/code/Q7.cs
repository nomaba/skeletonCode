    ...
    class Program
    {
        ...
        class Simulation
        {
            ...
            public void SetUpANestAt(int Row, int Column)
            {
                ...
                //CHANGE
                for (int Worker = 2; Worker <= StartingAntsInNest-1; Worker++)
                {
                    Ants.Add(new WorkerAnt(Row, Column, Row, Column));
                }
                Ants.Add(new ForagerAnt(Row, Column, Row, Column)); //Adds a single forager ant to each nest
                //END CHANGE
            }
            ...
            //CHANGE
            private int GetIndexOfNeighbourWithStrongestSmell(int Row, int Column)
            {
                int StrongestSmell = 0, IndexOfStrongestSmell = -1;
                foreach (int Index in GetIndicesOfNeighbours(Row, Column))
                {
                    if (Index != -1 && Grid[Index].GetSmell() > StrongestSmell)
                    {
                        IndexOfStrongestSmell = Index;
                        StrongestSmell = Grid[Index].GetSmell();
                    }
                }
                    ...
                    foreach (Ant A in Ants)
                    {
                        ...
                        else
                        {
                            ...
                            A.ChooseCellToMoveTo(GetIndicesOfNeighbours(A.GetRow(), A.GetColumn()),
                                                    GetIndexOfNeighbourWithStrongestPheromone(A.GetRow(), A.GetColumn()), GetIndexOfNeighbourWithStrongestSmell(A.GetRow(), A.GetColumn()));
                            //END CHANGE
                        }
                    }
                    foreach (Nest N in Nests)
        ...
        class Cell : Entity
        {
            ...
            {
                return $"{base.GetDetails()}{AmountOfFood} food present, smell: {Smell}{Environment.NewLine}{Environment.NewLine}";
            }
            ...
            public void UpdateFoodInCell(int Change)
            {
                ...
                if (AmountOfFood < 0)
                {
                    AmountOfFood = 0; //Ensure food doesn't go negative
                }
                //Update smell based on the amount of food
                Smell = AmountOfFood * 10; 
            }
            //END CHANGE
        }
        ...
        class Ant : Entity
        {
            ...
            //CHANGE
            public virtual void ChooseCellToMoveTo(List<int> ListOfNeighbours, int IndexOfNeighbourWithStrongestPheromone, int IndexOfNeighbourWithStrongestSmell = -1)
            {
            }
            //END CHANGE
        ...
        class WorkerAnt : Ant
        {
            ...
            //CHANGE
            public override void ChooseCellToMoveTo(List<int> ListOfNeighbours, int IndexOfNeighbourWithStrongestPheromone, int IndexOfNeighbourWithStrongestSmell = -1)
            {
            //END CHANGE
        ...
        {
            private Dictionary<int, string> SmellDirection = new Dictionary<int, string>();
            public ForagerAnt(int StartRow, int StartColumn, int NestInRow, int NestInColumn)
                : base(StartRow, StartColumn, NestInRow, NestInColumn)
            {
                TypeOfAnt = "forager";
                FoodCapacity = 30;
                SmellDirection.Add(0, "North-West");
                SmellDirection.Add(1, "North");
                SmellDirection.Add(2, "North-East");
                SmellDirection.Add(3, "West");
                SmellDirection.Add(5, "East");
                SmellDirection.Add(6, "South-West");
                SmellDirection.Add(7, "South");
                SmellDirection.Add(8, "South-East");
            }
            ...
            public override void ChooseCellToMoveTo(List<int> ListOfNeighbours, int IndexOfNeighbourWithStrongestPheromone, int IndexOfNeighbourWithStrongestSmell = -1)
            {
                if (AmountOfFoodCarried > 0)
                {
                    Console.WriteLine("Forager has food and is heading back to the nest");
                    if (Row > NestRow)
                    {
                        Row--;
                    }
                ...
                else if (IndexOfNeighbourWithStrongestSmell != -1)
                {
                    int IndexToUse = ListOfNeighbours.IndexOf(IndexOfNeighbourWithStrongestSmell);
                    Console.WriteLine($"Forager Ant is moving towards the strongest smell coming from the {SmellDirection[IndexToUse]}");
                    ChangeCell(IndexToUse, ref Row, ref Column);
                }
                else
                {
                    Console.WriteLine("No smell detected by forager, moving randomly...");
                    int IndexToUse = ChooseRandomNeighbour(ListOfNeighbours);
                    ChangeCell(IndexToUse, ref Row, ref Column);
                }
            }
