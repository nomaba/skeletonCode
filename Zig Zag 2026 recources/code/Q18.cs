...
namespace AntSimCS      //Q18
{
    class Program
    {
        ...
        static void Main()
        {
            ...
            do
            {
                ...
                switch (Choice)
                {
                    ...
                    //CHANGE
                    case "6":
                        Console.WriteLine(ThisSimulation.GetDangerPheromoneLocations());
                        break;
                        //END CHANGE
                }
            } while (Choice != "9");
        ...
        static void DisplayMenu()
        {
            ...
            //CHANGE
            Console.WriteLine("6. Display Nests with dangerously low food levels");
            //END CHANGE
        ...
        class Simulation
        {
            ...
            //CHANGE
            public bool IsDangerPheromoneInCell(Cell C)
            {
                foreach (Nest N in Nests)
                {
                    if (N.InDanger() && N.InSameLocation(C))
                    {
                        return true;
                    }
                }
            ...
            public string GetDangerPheromoneLocations()
            {
                String Details = "";
                foreach (Nest N in Nests)
                {
                    if (N.InDanger())
                    {
                        Details += $"Nest at ({N.GetRow()}, {N.GetColumn()}) has dangerously low food levels.{Environment.NewLine}";
                    }
                }
                return Details;
            }
            ...
            public string GetDetails()
            {
                ...
                for (int Row = 1; Row <= NumberOfRows; Row++)
                {
                    for (int Column = 1; Column <= NumberOfColumns; Column++)
                    {
                        ...
                        //CHANGE
                        if (IsDangerPheromoneInCell(TempCell))
                        {
                            Details += "| Nest has dangerously low food levels |  ";
                        }
                        //END CHANGE
            ...
            public string GetCellDetails(int Row, int Column)
            {
                ...
                if (N != null)
                {
                    ...
                    //CHANGE
                    if (IsDangerPheromoneInCell(CurrentCell))
                    {
                        Details += "| Nest has dangerously low food levels |  ";
                    }
                    //END CHANGE
            ...
            public void AdvanceStage(int NumberOfStages)
            {
                for (int Count = 1; Count <= NumberOfStages; Count++)
                {
                    ...
                    foreach (Nest N in Nests)
                    {
                        ...
                        //CHANGE
                        if (N.GetFoodLevel() < 50 && !N.InDanger())
                        {
                            N.SetInDanger(true);
                        }
                        else
        ...
        class Nest : Entity
        {
            ...
            //CHANGE
            public void SetInDanger(bool status)
            {
                if (status)
                {
                    D = new DangerPheromone(GetRow(), GetColumn(), 0, 100, 10);
                }
                else
                {
                    D = null;
        ...
        {
            public DangerPheromone(int Row, int Column, int BelongsToAnt, int InitialStrength, int Decay)
                : base(Row, Column, BelongsToAnt, InitialStrength, Decay)
            {
                BelongsTo = BelongsToAnt;
                Strength = InitialStrength;
                PheromoneDecay = Decay;
