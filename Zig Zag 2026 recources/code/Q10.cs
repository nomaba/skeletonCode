...
namespace AntSimCS  //Q10
{
    class Program
    {
        ...
        class Simulation
        {
            ...
            private Weather CurrentWeather; //Only one weather event can exist at a time
            ...
            //CHANGE
            private int GetIndexOfNeighbourWithBadWeather(int Row, int Column)
            {
                int IndexOfBadWeather = -1;
                foreach (int Index in GetIndicesOfNeighbours(Row, Column))
                {
                    if (Index != -1 && GetIfBadWeatherInCell(Grid[Index]) == true)
                    {
                        IndexOfBadWeather = Index;
                    }
                }
                return IndexOfBadWeather;
            }
            //END CHANGE
            ...
            //CHANGE
            public bool GetIfBadWeatherInCell(Cell C)
            {
                if (CurrentWeather != null && CurrentWeather.InSameLocation(C))
                {
                    return true;
                }
                return false;
            ...
            public string GetDetails()
            {
                ...
                for (int Row = 1; Row <= NumberOfRows; Row++)
                {
                    for (int Column = 1; Column <= NumberOfColumns; Column++)
                    {
                        ...
                        {
                            if (CurrentWeather.GetRow() == Row && CurrentWeather.GetColumn() == Column)
                            {
                                Details += "| Weather |";
                            }
                        }
            ...
            public void AdvanceStage(int NumberOfStages)
            {
                ...
                {
                    //CHANGE
                    List<int> Chances = new List<int>() { 0, 1, 2 };
                    if (CurrentWeather == null && Chances.Contains(RGen.Next(0, 4)))
                    {
                        int WeatherRow = RGen.Next(1, NumberOfRows + 1);
                        int WeatherColumn = RGen.Next(1, NumberOfColumns + 1);
                        CurrentWeather = new Weather(WeatherRow, WeatherColumn);
                        Console.WriteLine($"A weather event has occurred at: {WeatherRow},{WeatherColumn}");
                    }
                    //END CHANGE
                    ...
                    foreach (Ant A in Ants)
                    {
                        ...
                        else
                        {
                            ...
                            A.ChooseCellToMoveTo(GetIndicesOfNeighbours(A.GetRow(), A.GetColumn()),
                                                 ...
                                                 GetIndexOfNeighbourWithBadWeather(A.GetRow(), A.GetColumn()));
                        }
                        //END CHANGE
                    }
                    foreach (Nest N in Nests)
                    ...
                    if (CurrentWeather != null)
                    {
                        //The Impact of the weather needs to happen AFTER ants have moved around and been born / culled for this stage.
                        CurrentWeather.AdvanceStage(Nests, Ants, Pheromones);
                        if (CurrentWeather.IsExpired())
                        {
                            CurrentWeather = null; //Remove the weather event if it has expired
                            Console.WriteLine("The weather has passed...");
                        }
                    }
                    //END CHANGE
                }
        ...
        class Ant : Entity
        {
            ...
            //CHANGE
            public virtual void ChooseCellToMoveTo(List<int> ListOfNeighbours, int IndexOfNeighbourWithStrongestPheromone, int IndexOfBadWeather)
            {
            }
            //END CHANGE
        ...
        class WorkerAnt : Ant
        {
            ...
            //CHANGE
            public override void ChooseCellToMoveTo(List<int> ListOfNeighbours, int IndexOfNeighbourWithStrongestPheromone, int IndexOfBadWeather)
            {
                //The ants move back towards the nest even if there is weather there....
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        {
                            IndexToUse = ListOfNeighbours.IndexOf(IndexOfNeighbourWithStrongestPheromone);
                        }
                        else
                        {
                            IndexToUse = ChooseRandomNeighbour(ListOfNeighbours);
                            if (IndexToUse == IndexOfBadWeather)
                            {
                                continue;
                            }
                        }
                        int NewRow = Row, NewColumn = Column;
                        ChangeCell(IndexToUse, ref NewRow, ref NewColumn);
                    } while (IndexToUse == -1);
        ...
        {
            private int StagesRemaining;

            public Weather(int StartRow, int StartColumn) : base(StartRow, StartColumn)
            {
                StagesRemaining = 3;
            }
            ...
            {
                int AntsRemoved = 0;
                int PheromonesRemoved = 0;
                for (int i = Ants.Count - 1; i >= 0; i--)
                {
                    if (Ants[i].GetRow() == Row && Ants[i].GetColumn() == Column)
                    {
                        Ants.RemoveAt(i);
                        AntsRemoved++;
                    }
                ...
                {
                    if (Pheromones[i].GetRow() == Row && Pheromones[i].GetColumn() == Column)
                    {
                ...
                if (AntsRemoved > 0)
                {
                    Console.WriteLine($"The weather has removed {AntsRemoved} ants");
                }
                if (PheromonesRemoved > 0)
                {
                    Console.WriteLine($"The weather has removed {PheromonesRemoved} pheromones");
                }
                StagesRemaining--;
            }
