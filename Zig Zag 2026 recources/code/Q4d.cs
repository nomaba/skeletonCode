...
namespace AntSimCS          //Q4d
{
    class Program
    {
            ...
            do
            {
                ...
                switch (Choice)
                {
                    ...
                    case "2":
                        ...
                        //CHANGE 
                        GetCellReference(ref StartRow, ref StartColumn, ThisSimulation.GetSimulationWidth(),ThisSimulation.GetSimulationHeight(), "top left cell of the area you are interested in.");
                        GetCellReference(ref EndRow, ref EndColumn, ThisSimulation.GetSimulationWidth(), ThisSimulation.GetSimulationHeight(), "bottom right cell of the area you are interested in.");
                        Console.WriteLine(ThisSimulation.GetAreaDetails(StartRow, StartColumn, EndRow, EndColumn));
                        break;
                    case "3":
                        int Row = 0, Column = 0;
                        GetCellReference(ref Row, ref Column, ThisSimulation.GetSimulationWidth(), ThisSimulation.GetSimulationHeight(), "specific cell you are interested in.");
                        //END CHANGE
        ...
        //CHANGE
        static void GetCellReference(ref int Row, ref int Column, int MaxWidth, int MaxHeight, string DisplayMessage)
        {
            Console.WriteLine();
            ...
            while (!ValidHeight)
            {
                Console.WriteLine("Please enter in the cell reference for " + DisplayMessage);
                Console.Write("Enter row number: ");
                string RowAsString = Console.ReadLine();
                try
                {
                    Row = Convert.ToInt32(RowAsString);
                    ...
                    {
                        Console.WriteLine($"That is not a valid height. It must be between 1 and {MaxHeight} inc. Please try again");
                        Console.WriteLine();
                        continue;
                    }
                }
            ...
            while (!ValidWidth)
            {
                ...
                try
                {
                    Column = Convert.ToInt32(ColAsString);
                    ...
                    {
                        Console.WriteLine($"That is not a valid width. It must be between 1 and {MaxWidth} inc. Please try again");
                        Console.WriteLine();
                        continue;
                    }
                }
