    ...
    class Program
    {
        ...
        class WorkerAnt : Ant
        {
                ...
                : base(StartRow, StartColumn, NestInRow, NestInColumn)
            {
                TypeOfAnt = "worker";
                FoodCapacity = 30;
                MovementHistory = new Stack<Tuple<int, int>>();
                ForagingStages = 0;
            }
            //END CHANGE
            ...
            public override void ChooseCellToMoveTo(List<int> ListOfNeighbours, int IndexOfNeighbourWithStrongestPheromone)
            {
                ...
                {
		//CHANGE
                    ForagingStages = 0;         //Covers the scenario of ant finds food by luck while foraging
                ...
                else if (IndexOfNeighbourWithStrongestPheromone == -1)
                {
                    ...
                    //Console.WriteLine($"ForagingStages for this Ant: {ForagingStages}");
                    MovementHistory.Push(Tuple.Create(Row, Column));    //The ant is foraging whilst it can't smell a pheromone trail
                    ForagingStages++;          
                }
                else
                {
                    ...
                    ChangeCell(IndexToUse, ref Row, ref Column);
                    ForagingStages = 0;         //Once it finds a pheromone trail, the ant is no longer foraging.
                    MovementHistory.Clear();
                }
                //END CHANGE
            }
