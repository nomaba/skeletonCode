    ...
    class Program
    {
        ...
        class Simulation
        {
            ...
            public Simulation(List<int> SimulationParameters)
            {
                ...
                Birds.Add(new Bird(BirdRow, BirdColumn));
                //END CHANGE
            }
            ...
            //CHANGE
            public int GetNumberOfBirdsInCell(Cell C)
            {
                int Count = 0;
                foreach (Bird B in Birds)
                {
                    if (B.InSameLocation(C))
                    {
                        Count++;
                    }
                }
            ...
            public string GetDetails()
            {
                ...
                for (int Row = 1; Row <= NumberOfRows; Row++)
                {
                    for (int Column = 1; Column <= NumberOfColumns; Column++)
                    {
                        ...
                        //CHANGE
                        int AmountOfBirds = GetNumberOfBirdsInCell(TempCell);
                        if (AmountOfBirds > 0)
                        {
                            Details += $"| Birds: {AmountOfBirds} |  ";
                        }
                        //END CHANGE
            ...
            public void AdvanceStage(int NumberOfStages)
            {
                for (int Count = 1; Count <= NumberOfStages; Count++)
                {
                    ...
                    foreach (Bird B in Birds)
                    {
                        B.AdvanceStage(Nests, Ants, Pheromones);
                        B.ChooseCellToMoveTo(NumberOfRows + 1, NumberOfColumns + 1);
                    }
                    //END CHANGE
                }
            }
        ...
        class Entity
        {
            ...
            //CHANGE
            public virtual void ChooseCellToMoveTo(int NumberOfRows, int NumberOfColumns)
            {
            }
            //END CHANGE
        }
        ...
        class Bird : Entity
        {
            public Bird(int StartRow, int StartColumn) : base(StartRow, StartColumn)
            {
            }
            ...
            public override void AdvanceStage(List<Nest> Nests, List<Ant> Ants, List<Pheromone> Pheromones)
            {
                ...
                foreach (Ant A in Ants)
                {
                    ...
                    {
                        AntsToRemove.Add(A);
                        if (AntsToRemove.Count == 5)
                        {
                            break;
                        }
                    }
                }
                if (AntsToRemove.Count > 0)
                {
                    Console.WriteLine($"Bird at ({Row}, {Column}) has eaten {AntsToRemove.Count} ant(s)!");
                }

                foreach (Ant A in AntsToRemove)
                {
                    Ants.Remove(A);
                }
            }

            private void ChangeCell(int Direction, ref int RowToChange, ref int ColumnToChange)
            {
                if (Direction > 5)
                {
                    RowToChange++;
                ...
                if (new int[] { 0, 3, 6 }.Contains(Direction))
                {
                    ColumnToChange--;
                }
                else if (new int[] { 2, 5, 8 }.Contains(Direction))
                {
                    ColumnToChange++;
                }
            }

            public override void ChooseCellToMoveTo(int NumberOfRows, int NumberOfColumns)
            {
                ...
                while (!ValidLocation)
                {
                    Direction = RGen.Next(0, 9);
                    ChangeCell(Direction, ref NewRow, ref NewColumn);
