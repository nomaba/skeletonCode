...
namespace AntSimCS          //Q11
{
    class Program
    {
        ...
        static void Main()
        {
            ...
            do
            {
                ...
                switch (Choice)
                {
                    ...
                    //CHANGE
                    case "6":
                        ThisSimulation.DisplayFoodDeliveryData(); 
                        break;
                    //END CHANGE
                }
            } while (Choice != "9");
        ...
        static void DisplayMenu()
        {
            ...
            //CHANGE
            Console.WriteLine("6. Display food delivery data");
            //END CHANGE
        ...
        class Simulation
        {
            ...
            //CHANGE
            // Dictionary to store the amount of food delivered by each ant
            private Dictionary<int, int> FoodDeliveredByAnt = new Dictionary<int, int>();
            //END CHANGE
                ...
                for (int Count = 1; Count <= NumberOfStages; Count++)
                {
                    ...
                    foreach (Ant A in Ants)
                    {
                        ...
                        if (A.GetFoodCarried() > 0 && A.IsAtOwnNest())
                        {
                            ...
                            //CHANGE
                            if (!FoodDeliveredByAnt.ContainsKey(A.GetID()))
                            {
                                FoodDeliveredByAnt[A.GetID()] = A.GetFoodCarried();//If the ant ID is not in the dictionary
                            }
                            else
                            {
                                FoodDeliveredByAnt[A.GetID()] += A.GetFoodCarried();
                            }                                
                            //END CHANGE
            ...
            //CHANGE
            public void DisplayFoodDeliveryData()
            {
                Console.WriteLine("Ant ID\tFood Delivered");
                Console.WriteLine("-----------------------");
                int TotalFood = 0;
                foreach (KeyValuePair<int,int> Entry in FoodDeliveredByAnt)
                {
                    Console.WriteLine($"{Entry.Key}\t{Entry.Value}");
                    TotalFood += Entry.Value;
                }
                if (FoodDeliveredByAnt.Count > 0)
                {
                    Console.WriteLine("-----------------------");
                    double AverageFood = TotalFood / FoodDeliveredByAnt.Count;
                    Console.WriteLine($"Average Food Delivered: {AverageFood:F2} units per ant");
                }                
                Console.WriteLine("-----------------------");
