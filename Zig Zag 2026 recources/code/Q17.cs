...
namespace AntSimCS      //Q17
{
    class Program
    {
        ...
        static void Main()
        {
            ...
            do
            {
                ...
                switch (Choice)
                {
                    ...
                    //CHANGE
                    case "6":
                        int RowToInfect = 0, ColumnToInfect = 0;
                        GetCellReference(ref RowToInfect, ref ColumnToInfect);
                        Console.WriteLine(ThisSimulation.InfectAntsInCell(RowToInfect, ColumnToInfect));
                        break;
                        //END CHANGE
        ...
        static void DisplayMenu()
        {
            ...
            //CHANGE
            Console.WriteLine("6. Infect Ants in a cell");
            //END CHANGE
        ...
        class Simulation
        {
            ...
            //CHANGE
            public string InfectAntsInCell(int Row, int Column)
            {
                int TotalNumberOfAntsInfectedInCell = 0;
                foreach (Ant A in Ants)
                {
                    if (A.GetRow() == Row && A.GetColumn() == Column && A.GetTypeOfAnt() == "worker" && !A.IsDiseased())
                    {
                        A.Infect();
                        TotalNumberOfAntsInfectedInCell++;
                    }
                }
                return $"Total number of ants infected in cell {Row}, {Column}: {TotalNumberOfAntsInfectedInCell}{Environment.NewLine}";
            }
            //END CHANGE
        ...
        class Ant : Entity
        {
            ...
            //CHANGE
            protected bool Diseased = false;
            protected int StagesSinceInfection = 0;
            //END CHANGE
            ...
            {
                Diseased = true;
                StagesSinceInfection = 0;
            }
            public bool IsDiseased()
            {
                return Diseased;
            }
            public int GetStagesSinceInfection()
            {
                return StagesSinceInfection;
            }
            //END CHANGE
        ...
        class Nest : Entity
        {
            ...
            public override void AdvanceStage(List<Nest> Nests, List<Ant> Ants, List<Pheromone> Pheromones)
            {
                ...
                //CHANGE
                int NumberOfAntsDiedThroughDisease = 0;
                for (int i = 0; i < Ants.Count; i++)
                {
                    if (Ants[i].GetTypeOfAnt() == "worker" && Ants[i].IsDiseased() && Ants[i].GetStagesSinceInfection() >= 10)
                    {
                        Ants.RemoveAt(i);
                        i--;
                        NumberOfAntsDiedThroughDisease++;
                    }
                }
                if (NumberOfAntsDiedThroughDisease > 0)
                {
                    Console.WriteLine($"{NumberOfAntsDiedThroughDisease} have died through disease");
                }
                //END CHANGE
                foreach (Ant A in Ants)
                {
