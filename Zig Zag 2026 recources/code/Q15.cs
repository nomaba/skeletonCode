...
namespace AntSimCS      //Q15
{
    class Program
    {
        ...
        static void Main()
        {
            ...
            do
            {
                ...
                switch (Choice)
                {
                    ...
                    //CHANGE
                    case "6":
                        ThisSimulation.GetColonySummary();
                        break;
                }
            } while (Choice != "9");
            Console.ReadLine();
        ...
        static void DisplayMenu()
        {
            ...
            //CHANGE
            Console.WriteLine("6: Display Colony Stats");
            //END CHANGE
        ...
        class Simulation
        {
            ...
            //CHANGE
            public void GetColonySummary()
            {
                int TotalAnts = Ants.Count;
                int AntsCarryingFood = 0;
                int TotalFoodInNests = 0;
                int TotalStagesAlive = 0;

                //Count ants carrying food and calculate total stages alive
                foreach (Ant A in Ants)
                {
                    if (A.GetFoodCarried() > 0)
                    {
                    ...
                    TotalStagesAlive += A.GetStages(); 
                }

                //Calculate total food in all nests
                foreach (Nest N in Nests)
                {
                    TotalFoodInNests += N.GetFoodLevel();
                }

                //Calculate average age of ants
                double AverageAge = 0;
                if (TotalAnts > 0)
                {
                    AverageAge = TotalStagesAlive/TotalAnts;
                }

                //Display the summary
                Console.WriteLine("Colony Summary:");
                Console.WriteLine($"Total Ants: {TotalAnts}");
                Console.WriteLine($"Ants Carrying Food: {AntsCarryingFood}");
                Console.WriteLine($"Total Food in Nests: {TotalFoodInNests}");
                Console.WriteLine($"Average Age of Ants: {AverageAge:F2} stages");
            }
            //END CHANGE
        }
